...
class HomeAffairsPipeline(object):
    def process_item(self, item, spider):
        # 必要なシートだけCSVで保存する
        path = os.path.join(spider.save_dir, item['dat_path'])
        csv_path = os.path.join(spider.save_dir, "CityCode_{}.csv".format(datetime.strftime(datetime.now(), '%Y%m%d_%H%M')))
        info = pd.read_excel(path, sheet_name=0, dtype=str)
