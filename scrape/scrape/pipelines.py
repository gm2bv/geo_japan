...
#        print('PIPELINE', item['pref_id'], item['fname'])
        ...
        def gen_infos(row):
            ret = [row['town']]
#            print(ret)
            if row['town_num'] == row['town_num'] and row['town_num']:
                ret.append(str(int(row['town_num'])))
#            print(ret)
            if row[2] == row[2] and row[2]:
                ret.append(str(row[2]))
#            print(ret)
            if row[3] == row[3] and row[3]:
                ret.append(str(row[3]))
#            print(ret)
            return " ".join(ret)
        
        dat['town_infos'] = dat.apply(gen_infos, axis=1)

        city_id = item['fname'].split('.')[0]
        output_path = os.path.join(self.output_dir, "{}_{}.csv.gz".format(item['pref_id'], city_id))
        cols = [6, 7]  # 緯度,経度
        dat[['pref_name', 'city_name', 'town_infos'] + cols].to_csv(output_path, index=False, header=None, compression='gzip')
#        cols = [2, 3, 6, 7]  # 番地,号,緯度,経度
#        dat[['pref_name', 'city_name', 'town', 'town_num', 's_town'] + cols].to_csv(output_path, index=False, header=None, compression='gzip')
        del dat
