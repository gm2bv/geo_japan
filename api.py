...
#        raise exceptions.NotFound("指定の都道府県・市区が見つかりません")
        ...
        pref_id = pref_city[0].values[0]
#        city_id = pref_city[1].values[0]
        pc_path = os.path.join("tools", "output", "{:02}_2018.csv".format(int(pref_id)))
        if not os.path.exists(pc_path):
            raise exception.NotFound()

    print("DETAIL", is_detail, dat.shape)
    # データ読込＆解析＆抽出
    # pref, city, town, town_num, words, num1, num2 ,lati, lngi
    dat = pd.read_csv(pc_path, compression='gzip', header=None)
    dat = dat[(dat[0]==_pref) & (dat[1]==_city)]
    ...
    if len(rest_words[num+1:]) > 0:
        ...
        i = 0
        while len(_target) == 0:
            _infos = num_infos[:i] if i < 0 else num_infos
            if len(_infos) <= 0:
                break
            _target = select_target(target, _infos)
            i -= 1
...
def select_target(target, num_infos):
    if len(num_infos) == 3:
        _target = target[(target[3] == num_infos[0]) & (target[5] == num_infos[1]) & (target[6] == num_infos[2])]
        ...
        # あり得ない分岐
        raise Exception("num_infosが不正です")
